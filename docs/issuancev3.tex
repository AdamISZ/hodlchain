...
\title{\textbf{HODLcoin}\\[0.5em]\large or, a final resolution to the infinite argument over whether saving in bitcoin is a form of using bitcoin.}
...
Somsen's earlier work on spacechains had two essential components: burning coins on L1 (Bitcoin) and blind merged mining for consensus of L2, with the concept that the coins on L2, while not extractable back to L1, may hold value and may be swappable back to L1, contingent on the L2 surviving.

I considered that this framing was a bit ``off'' in a couple of ways. First, I think it's important to cleanly disassociate the issuance question from the consensus question. There are a ton of ideas about how to make L2 consensus work, from plain vanilla proof of work or proof of stake independent of L1, through SNARK based rollups and bridges, all the way to (blind) merged mining and drivechains; i.e. it's open to what extent you want to leverage existing consensus or even become part of it. The main part of the idea here, sets that question aside; but we \emph{will} address consensus at the end, offering plausible solutions for this specific new model.

Mainly here, we are going to address the question of: can the minting of coins on L2:

\begin{itemize}
\item be anchored: based on existing Bitcoin
\item be fair: not give privileged minting rights to any set of entities
\item be scarce: have supply be limited; at least asymptotically incrementally zero, if not perfectly finite
\item be permissionless: privileged entities should not control in any way the action of minting: it should be the sovereign bitcoin user themselves who performs the minting action, to prevent censorship or privacy loss
\end{itemize}

To abuse a horrible Claude-ism: ``the word "based" is doing a lot of the heavy lifting''. A classic holy-grail style L2 literally takes a Bitcoin utxo and interprets it as an L2 coin and then has some magical construction to ensure that the Bitcoin utxo can then only be spent in case of a succinct proof that the claimer is entitled to it, by L2 rules.

The design here is radically different: we intend to derive the value of the L2 coin from the \textbf{optionality} of spending the existing Bitcoin utxo.

As for scarcity and controlled supply, we are somewhat more relaxed, but derive an approximate control through something analogous to Bitcoin's difficulty retarget ratchet, as we shall see. The effect is that supply is strongly rate limited by how many Bitcoin exist, but that the coin created is not literally a ``parallel'' bitcoin, but rather a derivative of existing Bitcoin's value.

The core idea, in a nutshell: timelocked coins represent given-up optionality (the option to sell or transfer). This given up value is independently created and provable by any user, and so can be used as permission to mint new coins on an L2, henceforth ``hodlcoin'' for reasons that will become increasingly obvious over the course of this document.

Some immediate observations are worth making: 1/ supply is limited by the fact that Bitcoin utxos are finite in value. 2/ spacechains are essentially an asymptote of this idea: if you lock up your utxo for an infinite time, you should get the largest possible of that coin on the L2, since you have burnt them. 3/ no soft forks or other intrusions on L1 are necessary for this \emph{issuance} scheme; the consensus mechanism of L2 is another question that we'll address later 4/ there is a happy concordance with the ``hodling'' philosophy of many bitcoiners: any user other than you who chooses to use this method is reducing the supply of bitcoin available for sale, thus theoretically increasing the value of your ``hodlings'', and a more subtle but real effect: if \emph{most} of your ``usage'' of bitcoin is in the form of saving it (cue: endless arguments), then this system is also useful to you for the minor amount of spending you do: because if you lock up a large stash, you, for free, get an additional small stash of L2 coins to do your paltry amount of spending! How large/small is a matter of mathematics that we'll get into shortly.

One more issue in this early outline to mention: privacy and independence. The fourth bulletpoint raises something I think is crucial: it's very useful to be able to interact with such a system without someone else's intervention: for example, a centralized sequencer, or perhaps a swap provider and a market to trade. It's not just that a swap provider takes a cut --- which is often large, by the way! --- or that they might be down or the market might not be liquid, it's also something really important: most users want maximal privacy in their transactions. Achieving this is very non-trivial with Bitcoin, but in this system there is a path to making reasonably strong L1-L2 privacy: by choosing fixed timelocking periods, you can join an anonymity set such that your minting might be indistinguishable from the minting of a large anonymity set of other users. This is only a partially worked out idea, I won't completely develop it in this doc.

\subsection{Technical outline}
Here's a very brief summary of the (very simple) technical idea; you can skip this if you're not a Bitcoin engineer and go to the next section on comparisons, because you're probably wondering ``how is this different from X?''.

An L2 mints new native tokens to users who provably commit a Bitcoin taproot UTXO for a chosen duration via \texttt{OP\_CHECKSEQUENCEVERIFY} (CSV), with a per-mint reward $\fmint(V, T)$ that is superlinear in lock duration $T$ for moderate $T$ and saturates at the locked value $V$ as $T \to \infty$. The exact formula will be outlined and justified later, but for now note that the formula will have only one free variable $r$ representing an interest rate.

The taproot scriptPubKey contains an additional leaf with an unspendable Tagged Hash of the pubkey, where the tag is a label that specifies the L2. That means that this utxo is only authorized for \emph{this} L2, such as ``hodlcoin''.

The vanilla minting will just expose the pubkey and the timelock along with the outpoint, so that L2 nodes can reconstruct the scriptPubKey and verify valid minting. A ZKP will have to be developed, along with nullifiers to make this minting event k-anonymous for some fixed epoch like 3 months.

The final technical point is control of the issuance schedule. As mentioned, there is a parameter $r$, roughly an interest rate, that controls what amount of L2 is minted per L1 (time, value) lock. If issuance increases (it cannot be controlled, remember, since there is no entity with that power), $r$ decreases to make issuance less economically attractive; and vice versa. This, like bitcoin's difficulty retarget, allows a reasonably controlled supply, though not a fully controlled one. A reasonable target is a constant issuance per epoch (which asymptotically will reduce new issuance to near zero).

\subsection{Comparisons}

\begin{itemize}
\item \underline{Altcoins}: Later in the doc, we'll talk about ex-nihilo money creation. Bitcoin did this, but fairly: a lottery based on proof of work. Later altcoins did this less and less frequently and purely, eventually most of them used an ``issue to insiders and then sell'' model, mostly. But even the fairest are indeed creating money from nothing unless they are using proof of work. Energy supply is not infinite, therefore the security budget of coins is not infinite. This hodlcoin model does not create money from nothing: it takes the optionality of bitcoin holding and replaces it with a token: there is, in an abstract sense, a trade going on: by timelocking your coins, you are giving value to all other Bitcoin holders, who gain from having less selling competition (both if they want to sell, and if they don't, there is a gain to them here; bitcoins are literally more scarce during the timelock). Your trade is: I get an L2 coin I can use for $<$insert use case$>$ while I lose something I handed over to other Bitcoin holders: the right to sell for some other good.
\item \underline{Spacechains}: this is the easy one. As noted at the start, this idea is a development of that idea: spacechains are just the asymptotic case of hodlcoins: as $T \rightarrow \infty$, the value sacrificed is the entire coin. Indeed, one could easily imagine this as a preferred option in certain cases, \textbf{only if} the L2 has gained significant popularity/usage. The nice thing about hodlcoin is it's so low risk: you could get some usage while spending literally no bitcoin, only spending some very minor optionality that you quite likely wouldn't have used anyway. The consensus proposal (blind merged mining) in spacechains, we'll discuss nearer the end.
\item \underline{Utxo snapshots}: this comparison is instructive. You can create a new token/L2 by saying "every holder of bitcoin at date T gets the proportionate amount on L2". This has the fairness property but not the supply limit/scarcity property: since no coins are locked, there is no sacrificed value. Even one such "snapshot-coin", let alone 1M of them, represents ex-nihilo money creation. To be clear this is not saying ``you can't do that'' --- clearly, you can --- but it's arguing against the market's placing value on it.
\item \underline{Drivechains}: it's addressing the consensus part, which I have already said here, is not being addressed by this design; this design is focusing on what might be workable for the \textbf{issuance} question. Not to say that there isn't overlap; I'll later proposal a more simplistic consensus approach that doesn't require a soft fork. The part of drivechains that requires the soft fork is related to the problem of L2 to L1 exit, which hodlcoin very explicitly sidesteps.
\item \underline{Validity rollups}: again, it's not a direct comparison, but there are important overlaps to consider: rollups were more conceived for chains like Ethereum, in which state transitions are more easily enforced directly with L1 validation. In Bitcoin, that isn't possible, except with BitVM et al as per the next item, but curiously this doesn't matter as much as you'd think: if a state transition is succinctly verifiable by even light clients or full nodes on L2, then a cheating sequencer does not profit from publishing garbage state update attestations on L1, while the main point of attestation: borrowing from Bitcoin's double spend prevention, protected by proof of work, is still there. We'll discuss this more later in the doc, but for now, note that the other part of the concept of a rollup: using a DA layer to publish concise transaction representations, is somewhat orthogonal: it can be done in this hodlcoin construction, with a state root committed to L1 trivially in OP\_RETURN or similar.
\item \underline{BitVM and friends}: BitVM and Glock are hyper-focused on the no-L1-changes solution to the near-insoluble problem of ``verify L2 state on L1''. As such they (attempt to) solve an extremely important problem as trustlessly as possible; even where they are practical (arguably Glock/Mosaic are starting to approach that), they have to trade off on trust a little bit: signing committees in the absence of covenants in Bitcoin Script, and probably more importantly, named Operator entities to execute withdrawals - this element is probably the biggest weakness of the idea, since exit implies forming relationships with Operators (or being one!).

\end{itemize}

It might be worth mentioning the trivially true: any such L2 can be exited out of via atomic swap actions, assuming you can find a counterparty. (As was observed in spacechains, and later in BitVM also). This could be seen as the ``obvious'' way to participate if you only care about the economics, not the payment functionality: mint the L2 coins by locking, then immediately swap out of L2 to L1 for a (probably very small) extra L1 profit. This is not a flaw: it's a natural market mechanism for people to express their preference for L1 holding over whatever functionality L2 offers (such as private or cheap transactions or etc.).

What's the biggest trade-off that the hodlcoin idea makes? Simple, it's that you \uline{only gain access to a small fraction of your bitcoin's value, unless you trust the L2 so much that you want to move all your capital across permanently}. That's why ``hodlcoin'' an otherwise silly name, fits particularly well. That slider of 'a small fraction' is to my mind an enormous advance on the earlier Spacechains idea.
...
\begin{itemize}[leftmargin=*]
  ...
  \item The L2 token minted is not BTC, and is not pegged to BTC. It is a separate token whose issuance is metered against demonstrated, time-bounded BTC commitment. Its value is doubtless \emph{strongly correlated} with BTC value, but it's not the same thing.
  \item There is no committee, federation, or threshold quorum involved in the issuance mechanism.
\end{itemize}

\section{Philosophical Implications of Issuance}
\label{sec:philosophy}

In the introduction, we mentioned the question of ex-nihilo money creation in different models, and argued that it doesn't apply here. But that's a nontrivial point so I think it's worth exploring in detail. Here goes.
